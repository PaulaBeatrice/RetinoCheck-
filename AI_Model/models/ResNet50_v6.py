...
from tensorflow.keras.layers import (Dropout, Flatten, Dense, MaxPooling2D, Input, Conv2D, GlobalAveragePooling2D)
...
os.listdir('D:\FACULTATE\FACULTATE\LICENTA\proiect\data\APTOS_DDR')
df_train = pd.read_csv(r'D:\FACULTATE\FACULTATE\LICENTA\proiect\data\APTOS_DDR\train_aug.csv')
df_train.head()
...
for i, image_id in enumerate(tqdm(df_train['id_code'])):
    image = cv2.imread(
        f'D:/FACULTATE/FACULTATE/LICENTA/proiect/data/APTOS_DDR/train_images_aug/{image_id}.png')
...
RLROP_PATIENCE = 3
DECAY_DROP = 0.5
DETAILS = "augmentation applied, ADDING LAYERS, aptos+ddr"
VERSION = "6"
